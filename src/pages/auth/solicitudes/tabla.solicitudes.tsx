import { Badge, IconButton, Text, Button, Stack, Tooltip } from "@chakra-ui/react";
...
function TablaSolicitudes() {
  ...
  const {setForm,lista,setLista,filtros,conteo} = useSolicitudes()
  ...
  const list = lista.map((e : solicitudesData) => ({
    ...
    return <>
    <Stack direction='row' justifyContent='center' alignItems='center' gap={12} paddingBottom={4}>
    <Text fontSize='small' as='b' color='tomato' >Total: {conteo} </Text>
    ...
    <Column
      ...
      cellRenderer={({cellData : e})=>(
        <Badge colorScheme={retornaColor(e)}><small>{e}</small></Badge> 
      )}
      />
    <Column
      dataKey='solicito'
      ...
      width={78}
      cellRenderer={({cellData : e})=>(
        e.tipo == 0 ? <Badge><small>{e.solicito}</small></Badge> : '' 
      )}
      />
    <Column
      dataKey='farma'
      ...
      cellRenderer={({cellData :e })=>(
        <Stack direction='row' gap={0.5}>
          ...
          <Tooltip placement="top" hasArrow label='Actualizar'>
            <IconButton onClick={()=>{actualizarSolicitud(e)}} aria-label="Ficha" variant='outline' size='xs' icon={<RepeatIcon color='green.500' />} />
          </Tooltip>
