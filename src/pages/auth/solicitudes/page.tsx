...
function SolicitudesPage() {
  ...
  const { loading, setLoading, setLista } = useSolicitudes();
  ...
  const actualizar = async () => {
    ...
    if (res.success) {
      swal({ title: "Iniciado", text: "El proceso se esta ejecutando en 2do plano.", icon: "success" });
    }
  };
      ...
      <Stack gap={2} spacing={2} mb={4} direction="column" justifyContent="center">
        ...
        <Stack direction={["column", "row"]} alignItems="center">
          ...
          <Stack direction="row" alignItems="center">
            <Button onClick={actualizar} leftIcon={<RepeatIcon />} colorScheme="teal" variant="outline" size="xs" isDisabled={disabled}>
              {disabled ? "Actualizando..." : "Actualizar"}
            </Button>
          </Stack>
        </Stack>
      </Stack>
