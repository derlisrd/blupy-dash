...
function ModalNoti() {
  ...
  const enviar = async () => {
    ...
    setLoading(true);
    const res = await APICALLER.enviarNoti({ token: dataUser.token, body: { id: form.user_id, body: texto, title: titulo } });
    setLoading(false);
    if (!res.success) {
  ...
  return (
    <Modal isOpen={modals.noti} onClose={onClose}>
      <ModalOverlay />
      <ModalContent>
